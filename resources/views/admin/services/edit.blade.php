...
<div class="container-fluid">
    ...
    <div class="row">
           <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">تعديل جديد</h4>
    ...
    <div class="row">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action=" {{route('services.update', ['service' => $obj->id])}}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-md-10">
                            <input class="form-control" placeholder="العنوان الرائيسي (عربي)" type="text" name="title[ar]"  value="{{ $obj->title['ar'] }}"  id="example-text-input">
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="example-text-input" class="col-md-2 col-form-label">العنوان الرائيسي (الانجليزية)</label>
                        <div class="col-md-10">
                            <input class="form-control" placeholder="العنوان الرائيسي ( الانجليزية)" type="text" name="title[en]" value="{{ $obj->title['en'] }}" id="example-text-input">
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="example-text-input" class="col-md-2 col-form-label">وصف   (عربي)</label>
                        <div class="col-md-10">
                            <textarea class="form-control" placeholder="وصف  (عربي)" type="text" name="body[ar]"  id="body_ar"> {{ $obj->body['ar'] }}</textarea>
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="example-text-input" class="col-md-2 col-form-label">وصف  (الانجليزية)</label>
                        <div class="col-md-10">
                            <textarea class="form-control" placeholder="وصف  (الانجليزية)" type="text" name="body[en]"  id="body_en">{{ $obj->body['en'] }}</textarea>
                        </div>
                    </div>
                    @foreach (['image' => 'الصورة الرئيسي'] as $field => $label)
                    <div class="mb-3">
                        <label class="form-label">{{ $label }}</label>
                        <input type="file" name="{{ $field }}" class="form-control">
                        @if ($obj->$field)
                            <div class="mt-2">
                                <img src="{{ asset('storage/' . $obj->$field) }}" style="max-height: 120px;">
                            </div>
                        @endif
                    </div>
                   @endforeach
